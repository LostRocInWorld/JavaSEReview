...
///**
// * @author Dustin_Peng
// * @Description JDK7 HASMMAP SOURCE CODE
// * @create 2022-10-07-03:03
// */
///*
...
//     */
//    transient java.util.HashMap.Node<K, V>[] table;//底层使用Node[]数组
//    /**
...
//     */
//    public HashMap() {//将加载因子赋值为默认的0.75
...
//        return putVal(hash(key)/*拿到哈希值*/, key, value, false, true);
//    }
//
//    /**
...
//                   boolean evict) {
//        java.util.HashMap.Node<K, V>[] tab;
//        java.util.HashMap.Node<K, V> p;//用来存放要放入的位置的元素
//        int n, i;
//        if ((tab = table) == null || (n = tab.length) == 0)
//            //第一次put时扩容
//            n = (tab = resize()).length;
//        if ((p = tab[i = (n - 1) & hash]) == null)//判读当前放入的位置元素是否为空
//            tab[i] = newNode(hash, key, value, null);//添加成功情况1
//        else {
//            java.util.HashMap.Node<K, V> e;
//            K k;
//            if (p.hash == hash &&
//                    ((k = p.key) == key || (key != null && key.equals(k))))//判断要放入的位置的元素的哈希值与新元素的哈希值是否一样并且它们的key是否真正相等
//                e = p;//满足就把当前要放入的位置的元素赋值给e
//            else if (p instanceof java.util.HashMap.TreeNode)
//                e = ((java.util.HashMap.TreeNode<K, V>) p).putTreeVal(this, tab, hash, key, value);
//            else {//哈希值不相等的情况或者哈希值一样但是key不一样，且当前放入的位置的元素不以红黑树存储
//                for (int binCount = 0; ; ++binCount) {
//                    if ((e = p.next) == null) {
//                        //新元素作为老元素的next
//                        p.next = newNode(hash, key, value, null);
//                        if (binCount >= TREEIFY_THRESHOLD - 1) // 链表存在的元素个数大于树化临界点8，变成红黑树的情况
//                            treeifyBin(tab, hash);
//                        break;
//                    }
//                    if (e.hash == hash &&
//                            ((k = e.key) == key || (key != null && key.equals(k))))//依次比较链表中元素的哈希值是否与新元素的哈希值一致并且两者的key是否真正相等
//                        break;//不用比较走到e!=null的判断中进入替换value
//                    p = e;//相当于循环迭代了，把当前位置的元素变成链表的元素
//                }
//            }
//            if (e != null) { // e存在，替换当前数组的元素，返回原有的value
//                V oldValue = e.value;
...
//        }
//        ++modCount;
//        if (++size > threshold)//添加完成后是不是要扩容了
//            resize();
...
//        int oldCap = (oldTab == null) ? 0 : oldTab.length;//第一次put时，oldCap为0
//        int oldThr = threshold;//第一次put扩容临界点也是0，不是第一次临界点为
//        int newCap, newThr = 0;
//        if (oldCap > 0) {
//            if (oldCap >= MAXIMUM_CAPACITY) {//是否旧数组容量为Integer的最大值是就返回就数组，不用扩容
//                threshold = Integer.MAX_VALUE;
//                return oldTab;
//            } else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
//                    oldCap >= DEFAULT_INITIAL_CAPACITY)//新数组容量为旧数组容量两倍且旧数组容量大于默认容量16
//                newThr = oldThr << 1; // 扩容临界点也扩大为原来两倍
//        } else if (oldThr > 0) // initial capacity was placed in threshold
//            newCap = oldThr;
//        else {               // 第一次put新容量为默认容量，新扩容临界点为默认扩容临界点为12
//            newCap = DEFAULT_INITIAL_CAPACITY;
//            newThr = (int) (DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);
//        }
//        if (newThr == 0) {
//            float ft = (float) newCap * loadFactor;//第一次完成后扩容临界点变为新容量
//            newThr = (newCap < MAXIMUM_CAPACITY && ft < (float) MAXIMUM_CAPACITY ?
//                    (int) ft : Integer.MAX_VALUE);
//        }
//        threshold = newThr;//重新赋值扩容临界点
//        @SuppressWarnings({"rawtypes", "unchecked"})
//        java.util.HashMap.Node<K, V>[] newTab = (java.util.HashMap.Node<K, V>[]) new java.util.HashMap.Node[newCap];
//        table = newTab;//第一次put初始化底层Node数组
//        if (oldTab != null) {
...
//        int n, index;
//        java.util.HashMap.Node<K, V> e;
//        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)//当前数组不为空且容量小于最小树化容量64
//            resize();
...
//    public Set<K> keySet() {
//        Set<K> ks;
//        return (ks = keySet) == null ? (keySet = new java.util.HashMap.KeySet()) : ks;
//    }
//
//    /**
